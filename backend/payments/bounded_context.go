...
// (see payments/adapter/stripe_acl.go), an asynchronous webhook
...
// ACL around the supplied Stripe client, and the application
// Service. Returns the application.BoundedContext envelope and the
// concrete *app.Service the layout / checkout adapter consume.
func New(db *sql.DB, client *stripe.Client) (application.BoundedContext, *app.Service) {
...
// pipeline.
func NewInMemory(client *stripe.Client) (application.BoundedContext, *app.Service) {
